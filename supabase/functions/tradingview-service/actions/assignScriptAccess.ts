...
export async function assignScriptAccess(
  ...
  try {
    ...
    console.log(`Attempting to assign script access: pine_id=${pine_id}, username=${tradingview_username}`);
    ...
    // Extract CSRF token
    const csrfMatch = scriptPageHtml.match(/name="csrfmiddlewaretoken"\s+value="([^"]+)"/);
    if (!csrfMatch) {
    ...
    console.log(`Username "${tradingview_username}" validated successfully`);
    ...
    console.log(`TradingView add access response status: ${addAccessResponse.status}`);

    if (!addAccessResponse.ok) {
      const errorText = await addAccessResponse.text();
      console.error('TradingView add access error:', errorText);
      throw new Error(`Failed to add script access: ${addAccessResponse.status}`);
    }

    const responseData = await addAccessResponse.json();
    console.log('TradingView add access response:', responseData);
    ...
    console.error('Script assignment error:', error);
